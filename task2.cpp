...
int main(int argc, char *argv[]) {
    ...
    if (ProcRank == GENERAL_PROCCESS) {
        ...
        for (size_t i = GENERAL_PROCCESS + 1; i < ProcNum; i++) {
            MPI_Recv(&LocalMax, 1, MPI_INT, MPI_ANY_SOURCE, ANY_TAG, MPI_COMM_WORLD,
                     &status);

            printf("\nTotal Max = %d, Local Max = %d from %d procces", TotalMax,
                   LocalMax, status.MPI_SOURCE);
